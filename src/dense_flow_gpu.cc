...
int main(int argc, char** argv){
  ...
  "{ w width       | 224 | output image width}"
  "{ h height      | 224 | output image height}"
  ;
  ...
  int DIM_X               = cmd.get<int>("width");
  int DIM_Y               = cmd.get<int>("height");
...
void ProcessClip( Video & v, toolbox::IOManager & io_manager, const int type, const int bound, const int DIM_X, const int DIM_Y ){
  ...
  v.read( clip, *max_span, true, DIM_X, DIM_Y );
  ...
  while( true ){
    v.read( clip, 1, true, DIM_X, DIM_Y );
